...
def load_data():
    ...
    df["total_no_reply"] = safe_sum_per_row(df, ["Didn’t Answer - Insta", "Didn’t Answer - Whats", "Didn’t Answer - TikTok"])
    return df
...
kpi5.metric("Didn't Answer", total_no_reply)
...
with tab_time:
    ...
    weekly_platform = st.selectbox("Choose platform (weekly view):", ["Instagram", "WhatsApp", "TikTok", "Calls"], index=0, key="weekly_platform")
    ...
    agg_cols = [weekly_cols_map["total"], weekly_cols_map["bookings"]]
    agg_cols = [c for c in agg_cols if c in df_weeks.columns]
    ...
    if agg_cols:
        ...
        with col_w1:
            ...
            if total_col in last_4.columns:
                ...
                chart = alt.Chart(chart_df).mark_bar().encode(x="Week:N", y="Value:Q", tooltip=["Week", "Value"])
    ...
    daily_platform = st.selectbox("Choose platform (last 7 days – daily view):", ["Instagram", "WhatsApp", "TikTok", "Calls"], index=0, key="last7_platform")
    ...
    if df_last7.empty:
        ...
        agg_cols = [daily_cols_map["total"], daily_cols_map["bookings"]]
        agg_cols = [c for c in agg_cols if c in df_last7.columns]

        if agg_cols:
            day_agg = df_last7.groupby(df_last7["Date"].dt.date)[agg_cols].sum().reset_index().rename(columns={"Date": "day"}).sort_values("day")
            ...
            with col_d1:
                ...
                if total_col in day_agg.columns:
                    ...
                    chart = alt.Chart(chart_df).mark_line(point=True).encode(x="Day:N", y="Value:Q", tooltip=["Day", "Value"])
                    st.altair_chart(chart, use_container_width=True)
                else:
